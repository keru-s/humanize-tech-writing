...
稳定的项目术语和行业术语应保留，即使它们来自隐喻，例如 `熔断`、`心跳`、`背压`、`流水线`、`冷启动`、`配置漂移`。对于没有稳定技术含义的表达，若隐喻只增加气势、包装感或压缩程度，没有增加技术精度，则优先改成实际对象、动作、状态和关系。

例如：

- `写路径收口为管理后台` → `配置只能通过管理后台写入`
- `盘点实锤 22 处字段差异` → `盘点确认有 22 处字段差异`
- `随时对账配置差异` → `随时比较配置差异`
- `在容器内取证：检查快照和数据库状态` → `在容器内检查快照和数据库状态`
...
- **2.2.0** — 增加“优先字面事实，降低隐喻解码负担”：识别需要二次还原的隐喻、宣传式修辞和压缩标签，基于上下文展开真实对象/动作/关系，同时保护 `熔断`、`心跳`、`配置漂移` 等稳定技术术语。
