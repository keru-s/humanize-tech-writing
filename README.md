...
很多难读的中文技术文字，不是因为英文概念本身有问题，而是因为把英文术语、搭配、noun phrase 或隐喻逐词映射成中文。每个字都认识，但组合起来不像自然中文，读者需要先猜回英文才能理解。

Skill 会区分：

- 已有成熟、自然的中文术语
- 项目明确采用的中文术语
- 只有英文概念存在、但中文直译并不自然的表达

例如：

- `single source of truth` 不机械翻成 `真相源`，可根据上下文写成 `唯一配置来源`、`以 X 为准` 等。
- `configuration drift` 不因为英文术语存在就默认保留 `配置漂移`；只表达差异时，`配置差异` 或 `配置不一致` 往往更直接。

没有成熟中文译法时，可以直接用普通中文说明实际含义；必要时保留英文原词并用自然中文解释，不为了“全中文”造生硬直译词。
...
```text
把 docs/design.md 里明显的 AI 生造词、黑话、英文直译腔和需要二次解码的隐喻改成工程师正常会写的表达。保留技术术语、事实和 MUST/SHOULD/MAY 的强弱语义。
```
...
原项目面向通用中文写作。本 fork 从 2.0.0 起转为技术写作专项 Skill；2.1.0 改成 **语义安全 + 文档类型感知 + 最小编辑** 的结构；2.2.0 增加 **英文直译检查 + 字面事实优先 + 隐喻解码负担检查**。

感谢原作者对中文 AI 写作模式的整理。本项目继续使用 MIT License。

# 版本历史

- **2.2.0** — 增加英文术语/搭配/隐喻的逐词直译检查，以及“优先字面事实，降低隐喻解码负担”：只保护项目明确术语和在中文技术语境中足够稳定的术语，不因英文存在对应概念而自动保留中文直译；同时保持核心 prompt 紧凑，避免用大量 few-shot 或坏词枚举替代判断规则。
