...
很多难读的中文技术文字，本质上是 **calque（逐词直译 / 仿译）**：英文概念、固定搭配或隐喻本身没有问题，但 LLM 把它逐词映射成中文后，句子变成“每个字都认识，组合起来却不像中文”。读者往往需要先猜原来的英文，才能理解作者真正想说什么。

Skill 会优先判断中文表达本身是否自然、清楚、约定俗成，而不是只看英文里有没有对应概念：

- 已有成熟中文术语：使用成熟译法。
- 项目明确采用某个中文术语：按项目约定保留。
- 没有成熟中文译法：优先用普通中文说明实际含义；必要时直接保留英文原词并用自然中文解释。
- 不会为了“全中文”而制造一个生硬的逐词直译词。

代表性例子：

- `single source of truth` 不机械翻成 `真相源`；根据上下文可写 `唯一配置来源`、`以 X 为准` 等。
- `configuration drift` 不因为英文术语存在就默认写 `配置漂移`；上下文只表达差异时，直接写 `配置差异` 或 `配置不一致` 更清楚。

### 优先字面事实，降低隐喻解码负担

“隐喻率”可以作为观察文风的信号，但 Skill 不使用机械阈值。真正检查的是：**读者是否必须先把修辞、黑话或形象标签翻译回字面含义，才能理解技术事实。**

项目明确使用的术语，以及在当前中文技术语境中足够稳定、精确的行业术语应保留。对于没有稳定技术含义的表达，如果隐喻只增加气势、包装感或压缩程度，没有增加技术精度，则优先改成实际对象、动作、状态和关系。
...
Skill 使用 plain language，优先普通词、具体动词、自然中文、字面事实和项目已有术语。
...
不会机械删除所有隐喻，也不会机械把英文术语翻成中文。判断标准是当前中文表达是否准确、自然，以及是否让读者多做一次“翻回英文 / 还原成技术事实”的脑内翻译。
...
```text
把 docs/design.md 里明显的 AI 生造词、黑话、英文逐词直译腔和需要二次解码的隐喻改成工程师正常会写的中文。保留技术术语、事实和 MUST/SHOULD/MAY 的强弱语义。
```
...
原项目面向通用中文写作。本 fork 从 2.0.0 起转为技术写作专项 Skill；2.1.0 改成 **语义安全 + 文档类型感知 + 最小编辑** 的结构；2.2.0 增加 **英文直译腔检查 + 字面事实优先 + 隐喻解码负担检查**。

感谢原作者对中文 AI 写作模式的整理。本项目继续使用 MIT License。

# 版本历史

- **2.2.0** — 增加英文逐词直译（calque）检查和“优先字面事实，降低隐喻解码负担”：不因英文存在对应概念就自动保留中文直译；识别需要二次还原的隐喻、宣传式修辞和压缩标签，并在有上下文依据时展开真实对象、动作和关系。
